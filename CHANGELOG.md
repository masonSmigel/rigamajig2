...
* Added deform layer manager UI
* Added confirm dialogs when building a new rig and doing a layered data save.
* Added `mayaMessageBox.py` for better modal message boxes.
* Added a GitHub dialog to help manage versions of files.
* Added support for blend shapes and other deformers to `deformLayer.DeformLayer.stackDeformLayers`
* Numerous UI updates:
    * updated header stretch on  `dataLoader`
    * updated readme with proper code to load the UI after workspace control update.
    * moved `sliderGrp` into general widgets
    * refactored the collapsable widget.
* Added support for nurbs in `skin_data.py`
* Major component data changes. Component parameters are now configured using the `defineComponentParameter` method.
  This also stores the expected datatype for each parameter and causes better error handling and a cleaner interface for
...
* Fixed a bug causing deformers not to load if no SHAPEs data is present. Fixed a bug causing deformers to load twice.
...
* Added a new UI widget called the `dataLoader` for managing layered data. This has been implemented on the Joints,
  Guides, Components and Controls.
    * left clicking the save button will save changes into the file the key originates from. new data will be appended
      to the last file.
...
* implemented a callback proxy for IKFK controls on `limb.limb` components and subclasses.This provides no difference to
  the
  animator but will eliminate a cycle caused by placing the IKFK control at the end of the chain. Using a scriptnode to
  setup a callback
  the selecting the proxy_selector will instead select the main ikfk control (located at a position safer for the eval
  graph)
...
* Added the option to change the logging level of all rigamajig loggers in the builder UI. This is super helpful for
  debugging.
...
* Changed the last joint of the joint changes created with `spline.addTwistJoints()` to match the name of the other
  joints.
...
* Changed the way components are loaded into the component manager from the scene. This update keeps the components in
  the
...
* Changed 'bind' and 'control' tags/suffixes and their uses to be based on a global variable within `common`. NOTE: Some
  functions
...
* Moved the PSD save and load functions of the builder Dialog to the `build` widget instead of the `deform` widget. This
  is
...
* Added the option to enable and disable components from building. This can be used to test and itterate on component
  setups.
...
* Changed all component parameters that requre a vector to use an axis direction instead. Its easier to understand and
  edit.
...
* Added a check to the `builder` to ensure the `main.main` component is always built first if its listed in the
  component list.
...
* Fixed a cycle cluster in the `eyelid.eyelid` by moving some attributes to the controls rather than driving the params
  Hierarchy.
...
* Added `mirrorComponentParameters` and `createMirroredComponent` options to the component manager.
  These funcitons aide in managing and creating new components.
    * `mirrorComponentParameters` will attempt to process and create a mirror name for all appropriate parameters in a
      component.
