...
class Eyelid(base.BaseComponent):
    ...
    def __init__(self, name, input, size=1, rigParent=str(), componentTag=None):
        ...
        self.eyelidSpans = 8
        self.addCrease = True
        self.addFleshyEye = True
        self.eyeballJoint = ""
        self.useCreaseFollow = False
        self.uppCreaseDriver = ""
        self.lowCreaseDriver = ""
        self.eyeSocketName = inputBaseNames[0]

        self.defineParameter(
            parameter="eyelidSpans", value=self.eyelidSpans, dataType="int"
        )
        self.defineParameter(
            parameter="addCrease", value=self.addCrease, dataType="bool"
        )
        self.defineParameter(
            parameter="addFleshyEye", value=self.addFleshyEye, dataType="bool"
        )
        self.defineParameter(
            parameter="eyeballJoint", value=self.eyeballJoint, dataType="string"
        )

        self.defineParameter(
            parameter="useCreaseFollow", value=self.useCreaseFollow, dataType="bool"
        )
        self.defineParameter(
            parameter="uppCreaseDriver", value=self.uppCreaseDriver, dataType="string"
        )
        self.defineParameter(
            parameter="lowCreaseDriver", value=self.lowCreaseDriver, dataType="string"
        )

        self.defineParameter(
            parameter="eyeSocketName", value=self.eyeSocketName, dataType="string"
        )
    ...
    def _createBuildGuides(self):
        ...
        self.uppLidGuideList, self.lowLidGuideList = self._createGuides(
            part="", startPos=socketPos
        )

        # create the eyelid control guides
        self.eyelidControllerGuides = self._createCurveGuides(
            part="", uppGuides=self.uppLidGuideList, lowGuides=self.lowLidGuideList
        )
        ...
        if self.addCrease:
            ...
            self.uppCreaseGuideList, self.lowCreaseGuideList = self._createGuides(
                part="Crease", startPos=socketPos, offset=1.5, color="lightorange"
            )
    ...
    def _createGuides(self, part="", startPos=[0.0, 0.0, 0.0], offset=1.0, color="turquoise"):
        ...
        parent = cmds.createNode(
            "transform",
            name="{}_{}_guide".format(self.name, part),
            parent=self.guidesHierarchy,
        )
        ...
        innCorner = control.createGuide(
            ...
            position=mathUtils.addVector(
                ((-midpoint - GUIDE_SCALE * offset) * sideMultipler, 0, 0), startPos
            ),
            size=guideSize,
            color=color,
        )
    ...
    def _createCurveGuides(self, part, uppGuides, lowGuides):
        ...
        uppGuideCurve = live.createLiveCurve(
            uppGuides, curveName=uppCurveName, parent=self.guidesHierarchy
        )

        lowCurveName = "{}_lower{}_guideCrv".format(self.name, part)
        lowGuideCurve = live.createLiveCurve(
            lowGuides, curveName=lowCurveName, parent=self.guidesHierarchy
        )
        ...
        lidControlHierarchy = cmds.createNode(
            "transform", name=hierarchyName, parent=self.guidesHierarchy
        )
        for i in range(len(eyelidControlNames)):
            suffix = eyelidControlNames[i]
            guide = control.createGuide(
                "{}_{}{}".format(self.name, suffix, part),
                shape="sphere",
                size=GUIDE_SCALE,
    ...
    def _initialHierarchy(self):
        ...
        self.eyeSocket = control.createAtObject(
            name=self.eyeSocketName,
            side=self.side,
            shape="eye",
            orig=True,
    ...
    def _preRigSetup(self):
        ...
        self.upVector = cmds.createNode(
            "transform", name="{}_upVec".format(self.name), parent=self.spacesHierarchy
        )
        ...
        self.topDriverCurve = curve.createCurveFromTransform(
        ...
        self.topLowCurve = cmds.duplicate(
            self.topDriverCurve, name=topControlCurveName
        )[0]
        cmds.rebuildCurve(self.topLowCurve, spans=4, degree=3, fitRebuild=True)

        self.botLowCurve = cmds.duplicate(
            self.botDriverCurve, name=botControlCurveName
        )[0]
        ...
        tempBlinkCurve = cmds.duplicate(self.topDriverCurve, name="self.tmpBlinkCurve")[
            0
        ]
        tempBshp = blendshape.create(
            tempBlinkCurve, targets=[self.topDriverCurve, self.botDriverCurve]
        )
        for target in blendshape.getTargetList(tempBshp):
            cmds.setAttr("{}.{}".format(tempBshp, target), 0.5)
        cmds.delete(tempBlinkCurve, constructionHistory=True)
        cmds.rebuildCurve(tempBlinkCurve, keepControlPoints=True, degree=3)
        ...
        self.blinkCurve = cmds.rebuildCurve(
            tempBlinkCurve,
            spans=4,
            degree=3,
        ...
        self.uppBlinkCurve = cmds.rebuildCurve(
            tempBlinkCurve,
            keepControlPoints=True,
            degree=1,
            replaceOriginal=False,
            name=topBlinkCurveName,
        )[0]
        self.lowBlinkCurve = cmds.rebuildCurve(
            tempBlinkCurve,
            keepControlPoints=True,
            degree=1,
            replaceOriginal=False,
            name=botBlinkCurveName,
        )[0]
        cmds.parent(
            self.blinkCurve,
            self.uppBlinkCurve,
            self.lowBlinkCurve,
            self.curvesHierarchy,
        )
        ...
        self.aimLocHierarchy = cmds.createNode(
            "transform", name="{}_aimTgts".format(self.name), parent=self.rootHierarchy
        )
        ...
        for guide in self.uppLidGuideList + self.lowLidGuideList[1:-1]:
            ...
            baseJoint = cmds.createNode(
                "joint", name="{}_baseTrs".format(guideName), parent=self.input[0]
            )
            transform.matchTranslate(self.input[0], baseJoint)

            endJoint = cmds.createNode(
                "joint", name="{}_bind".format(guideName), parent=baseJoint
            )
            ...
            joint.orientJoints(
                [baseJoint, endJoint], aimAxis="x", upAxis="y", autoUpVector=False
            )

            # connect the joints to the driver curves
            aimLoc = cmds.createNode(
                "transform",
                name="{}_trsAimTarget".format(guideName),
                parent=self.aimLocHierarchy,
            )
            transform.matchTranslate(guide, aimLoc)

            targetCurve = (
                self.botDriverCurve if "lower" in guideName else self.topDriverCurve
            )
        ...
        if self.addCrease:
            ...
            # create the low-res curves
            self.topCreaseLowCurve = cmds.duplicate(
                self.topCreaseDriverCruve, name=topCreaseControlCurveName
            )[0]
            cmds.rebuildCurve(
                self.topCreaseLowCurve, spans=4, degree=3, fitRebuild=True
            )

            self.botCreaseLowCurve = cmds.duplicate(
                self.botCreaseDriverCurve, name=botCreaseControlCurveName
            )[0]
            cmds.rebuildCurve(
                self.botCreaseLowCurve, spans=4, degree=3, fitRebuild=True
            )
            ...
            for guide in self.uppCreaseGuideList + self.lowCreaseGuideList[1:-1]:
                ...
                targetCurve = (
                    self.botCreaseDriverCurve
                    if "lower" in guideName
                    else self.topCreaseDriverCruve
                )
    ...
    def _rigSetup(self):
        ...
        if self.addCrease:
            wire1, _ = cmds.wire(
                self.topCreaseDriverCruve,
                wire=self.topCreaseLowCurve,
                dropoffDistance=(0, WIRE_DROPOFF),
                name="{}_wire".format(self.topDriverCurve),
            )
            wire2, _ = cmds.wire(
                self.botCreaseDriverCurve,
                wire=self.botCreaseLowCurve,
                dropoffDistance=(0, WIRE_DROPOFF),
    ...
    def setupBlink(self):
        ...
        wire1, _ = cmds.wire(
            self.topDriverCurve,
            wire=self.topLowCurve,
            dropoffDistance=(0, WIRE_DROPOFF),
            name="{}_wire".format(self.topDriverCurve),
        )
        wire2, _ = cmds.wire(
            self.botDriverCurve,
            wire=self.botLowCurve,
            dropoffDistance=(0, WIRE_DROPOFF),
            name="{}_wire".format(self.botDriverCurve),
        )
        wire3, _ = cmds.wire(
            self.uppBlinkCurve,
            wire=self.blinkCurve,
            dropoffDistance=(0, WIRE_DROPOFF),
        ...
        self.blinkBlendshape = blendshape.create(
            self.blinkCurve, name="{}_blink".format(self.blinkCurve)
        )
        blendshape.addTarget(
            self.blinkBlendshape, target=self.botLowCurve, targetWeight=1
        )
        blendshape.addTarget(
            self.blinkBlendshape, target=self.topLowCurve, targetWeight=1
        )
        ...
        blinkAttr = attr.createAttr(
            self.paramsHierarchy, "blink", "float", value=0, minValue=0, maxValue=1
        )
        heightAttr = attr.createAttr(
        ...
        lowBlinkMdl = node.multDoubleLinear(
            heightAttr, -1, name="{}_blinkHeightReverse".format(self.name)
        )
        lowBlinkAdl = node.addDoubleLinear(
            "{}.{}".format(lowBlinkMdl, "output"), 1, name="{}_height".format(self.name)
        )

        cmds.connectAttr(
            "{}.{}".format(lowBlinkAdl, "output"),
            "{}.{}".format(self.blinkBlendshape, bshpTargets[0]),
        )
        cmds.connectAttr(
            heightAttr, "{}.{}".format(self.blinkBlendshape, bshpTargets[1])
        )

        # create the blendshapes to trigger the blinks
        uppBlinkBshp = blendshape.create(
            self.topDriverCurve,
            targets=self.uppBlinkCurve,
            name="{}_blink".format(self.topDriverCurve),
        )
        lowBlinkBshp = blendshape.create(
            self.botDriverCurve,
            targets=self.lowBlinkCurve,
        ...
        cmds.connectAttr(
            blinkAttr,
            "{}.{}".format(uppBlinkBshp, blendshape.getTargetList(uppBlinkBshp)[0]),
        )
        cmds.connectAttr(
            blinkAttr,
            "{}.{}".format(lowBlinkBshp, blendshape.getTargetList(lowBlinkBshp)[0]),
        )
        ...
        tempSmooth = cmds.smoothCurve(
            "{}.cv[*]".format(self.blinkCurve), smoothness=10, constructionHistory=True
        )[-1]
        ...
        for i in interpDict.keys():
            cmds.setAttr(
                remapValue + ".value[{}].value_Position".format(i), interpDict[i][0]
            )
            cmds.setAttr(
                remapValue + ".value[{}].value_FloatValue".format(i), interpDict[i][1]
            )
            cmds.setAttr(
                remapValue + ".value[{}].value_Interp".format(i), interpDict[i][2]
            )

        mdl = node.multDoubleLinear(
            smoothness,
            "{}.{}".format(remapValue, "outValue"),
            name="{}_smooth".format(self.name),
        )
        cmds.connectAttr(
            "{}.{}".format(mdl, "output"), "{}.{}".format(smoothCurveNode, "smoothness")
        )
    ...
    def setupDriverCurve(self, controls, uppCurve, lowCurve, prefix=""):
        ...
        for ctl in controls:
            jnt = cmds.createNode(
                "joint", name=ctl.name + "_driver", parent=self.jointsHierarchy
            )
        ...
        cmds.skinCluster(
            ...
            maximumInfluences=2,
            bindMethod=0,
            name="{}_skinCluster".format(uppCurve),
        )
        cmds.skinCluster(
            lowLidJoints,
            lowCurve,
            ...
            maximumInfluences=2,
            bindMethod=0,
            name="{}_skinCluster".format(lowCurve),
        )
        ...
        uppLidFollow = attr.createAttr(
            self.paramsHierarchy,
            "upp{}TweakFollow".format(prefix),
            "float",
            minValue=0,
            maxValue=1,
            value=0.8,
        )
        lowLidFollow = attr.createAttr(
            self.paramsHierarchy,
            "low{}TweakFollow".format(prefix),
            "float",
            minValue=0,
            maxValue=1,
            value=0.8,
        ...
        blend1 = transform.blendedOffsetParentMatrix(
            controls[0].name, controls[2].name, controls[1].trs, mo=True
        )
        blend2 = transform.blendedOffsetParentMatrix(
            controls[4].name, controls[2].name, controls[3].trs, mo=True
        )
        blend3 = transform.blendedOffsetParentMatrix(
            controls[0].name, controls[6].name, controls[5].trs, mo=True
        )
        blend4 = transform.blendedOffsetParentMatrix(
            controls[4].name, controls[6].name, controls[7].trs, mo=True
        )
    ...
    def setupFleshyEye(self):
        ...
        if not cmds.objExists(self.eyeballJoint):
            raise Exception(
                "The eyeball joint '{}' does not exist".format(self.eyeballJoint)
            )
        ...
        if not aimAxis == "z" and not upAxis == "y":
            logger.warning("Eyeball joints should be oriented in world space.")

        fleshyEyeAttr = attr.createAttr(
            self.paramsHierarchy, "fleshyEye", "float", value=1, minValue=0, maxValue=1
        )
        ...
        fleshyEyeOffset = cmds.createNode(
            "transform",
            name="{}_fleshyOffset".format(self.name),
            parent=self.eyeSocket.name,
        )
        ...
        for part in ["upper", "lower"]:
            ...
            eyeReader = cmds.createNode(
                "transform", name=readerName, parent=fleshyEyeOffset
            )
            transform.matchTransform(self.eyeballJoint, eyeReader)

            followRX = attr.createAttr(
                self.paramsHierarchy,
                "{}FollowRx".format(part),
                "float",
                value=0.3,
                minValue=0,
                maxValue=1,
            )
            followRY = attr.createAttr(
                self.paramsHierarchy,
                "{}FollowRy".format(part),
                "float",
                value=0.3,
                minValue=0,
                maxValue=1,
            ...
            mult = node.multiplyDivide(
                "{}.r".format(self.eyeballJoint), [followRX, followRY, 0], name=multName
            )
            fleshySwitch = node.multiplyDivide(
                "{}.output".format(mult),
                [fleshyEyeAttr, fleshyEyeAttr, fleshyEyeAttr],
            ...
            eyelidControl = (
                self.lidControls[2] if part == "upper" else self.lidControls[6]
            )
            transform.connectOffsetParentMatrix(
                eyeReader, eyelidControl.trs, mo=True, t=True, r=False, s=False
            )
    ...
    def _setupAnimAttrs(self):
        ...
        attr.moveAttribute(
            "uppLidTweakFollow", self.paramsHierarchy, self.lidControls[2].name
        )

        # setup the lower lid
        attr.addSeparator(self.lidControls[6].name, "----")
        attr.moveAttribute(
            "lowLidTweakFollow", self.paramsHierarchy, self.lidControls[6].name
        )
        ...
        if self.addCrease:
            ...
            attr.moveAttribute(
                "uppCreaseTweakFollow",
                self.paramsHierarchy,
                self.creaseControls[2].name,
            )

            # setup the lower lid
            attr.addSeparator(self.creaseControls[6].name, "----")
            attr.moveAttribute(
                "lowCreaseTweakFollow",
                self.paramsHierarchy,
                self.creaseControls[6].name,
            )
    ...
    def _connect(self):
        ...
        if cmds.objExists(self.rigParent):
            ...
            transform.connectOffsetParentMatrix(
                self.rigParent, self.eyeSocket.orig, mo=True
            )

            # connect the up vector to the eyeSocket control
            transform.connectOffsetParentMatrix(
                self.eyeSocket.name, self.upVector, mo=True
            )
    ...
    def setupCreaseFollows(self):
        ...
        for part in ["upp", "low"]:
            creaseDriver = (
                self.uppCreaseDriver if part == "upp" else self.lowCreaseDriver
            )
            # if the string is empty then we can display a warning and skip this
            if creaseDriver == "":
                logger.warning("Must provide a crease driver to useCreaseFollow")
                return

            creaseHierachy = cmds.createNode(
                "transform",
                name="{}_{}CreaseFollow".format(self.name, part),
            ...
            lidTrs = cmds.createNode(
                "transform", name=lidTrsName, parent=creaseHierachy
            )
            cmds.setAttr("{}.inheritsTransform".format(lidTrs), False)

            lidControl = self.lidControls[2] if part == "upp" else self.lidControls[6]
            creaseControl = (
                self.creaseControls[2] if part == "upp" else self.creaseControls[6]
            )
            lidCurve = self.topDriverCurve if part == "upp" else self.botDriverCurve
            ...
            cmds.orientConstraint(self.rigParent, lidTrs, maintainOffset=True)
            ...
            creaseBiasAttr = attr.createAttr(
                creaseControl.name,
                "creaseBias".format(part),
                "float",
                value=0.5,
                minValue=0,
                maxValue=1,
            ...
            outputTrs = cmds.createNode(
                "transform", name=creaseTrsName, parent=creaseHierachy
            )
            transform.matchTransform(creaseControl.name, outputTrs)
            creaseBlend = transform.blendedOffsetParentMatrix(
                lidTrs, creaseDriver, outputTrs, mo=True, blend=0.5
            )
            ...
            cmds.connectAttr(
                ...
                "{}.offsetParentMatrix".format(creaseControl.trs),
                force=True,
            )

            # add the crease follow attribute to the lidControl.
            followAttr = attr.createAttr(
                creaseControl.name,
                "creaseFollow",
                "float",
                value=1,
