...
class Lips(base.BaseComponent):
    ...
    def __init__(self, name, input, size=1, rigParent=str(), componentTag=None):
        ...
        self.useJaw = False
        self.jawJoints = []
        self.addZipperLips = True
        self.sparseTweakers = True
    ...
    def _createBuildGuides(self):
        ...
        self.upperLipGuide = control.createGuide(
            name="{}_uppLipAll".format(self.name),
            parent=self.guidesHierarchy,
            hideAttrs=["s"],
            position=lipsPos,
            size=self.size * (GUIDE_SCALE * 2),
        )
        self.lowerLipGuide = control.createGuide(
            name="{}_lowLipAll".format(self.name),
            parent=self.guidesHierarchy,
            hideAttrs=["s"],
            position=lipsPos,
            size=self.size * (GUIDE_SCALE * 2),
        ...
        upperSubControls = self.createControlGuides(
            uppGuideCurve,
            uppSubControlNames,
            subControlParams,
            parent=subControlHierarchy,
            color="lightorange",
            size=0.5 * self.size,
        )
        lowerSubControls = self.createControlGuides(
            lowGuideCurve,
            lowSubControlNames,
            subControlParams,
            parent=subControlHierarchy,
    ...
    def _initialHierarchy(self):
        ...
        self.lipsAll = control.createAtObject(
            name=self.input[0].split("_")[0],
            side=self.side,
            shape="square",
            orig=True,
        ...
        self.uppLips = control.createAtObject(
            name="{}_uppAll".format(self.name),
            shape="square",
            orig=True,
            trs=True,
        ...
        self.lowLips = control.createAtObject(
            name="{}_lowAll".format(self.name),
            shape="square",
            orig=True,
            trs=True,
            parent=self.lipsAll.name,
    ...
    def _preRigSetup(self):
        ...
        uppPoints = [
            self.mainControlGuides[0],
            self.subControlGuides[1],
            self.mainControlGuides[1],
            self.subControlGuides[2],
            self.mainControlGuides[2],
            self.subControlGuides[3],
            self.mainControlGuides[3],
            self.subControlGuides[4],
            self.mainControlGuides[4],
        ]

        lowPoints = [
            self.mainControlGuides[0],
            self.subControlGuides[6],
            self.mainControlGuides[5],
            self.subControlGuides[7],
            self.mainControlGuides[6],
            self.subControlGuides[8],
            self.mainControlGuides[7],
            self.subControlGuides[9],
            self.mainControlGuides[4],
        ]

        # create the two driver curves. The jionts will be bound to this
        self.topHighCurve = curve.createCurveFromTransform(
            uppPoints, degree=3, name=topHighCurve, parent=self.curvesHierarchy
        )
        self.botHighCurve = curve.createCurveFromTransform(
            lowPoints, degree=3, name=botHighCurve, parent=self.curvesHierarchy
        )
    ...
    def setupMidCurve(self):
        ...
        if self.sparseTweakers:
            ...
            uppControlsForJoints = [
                self.subControls[0],
                self.subControls[1],
                self.extraTweakers[0],
                self.subControls[2],
                self.extraTweakers[1],
                self.subControls[3],
                self.extraTweakers[2],
                self.subControls[4],
                self.subControls[5],
            ]
            lowControlsForJoints = [
                self.subControls[0],
                self.subControls[6],
                self.extraTweakers[3],
                self.subControls[7],
                self.extraTweakers[4],
                self.subControls[8],
                self.extraTweakers[5],
                self.subControls[9],
                self.subControls[5],
            ]
