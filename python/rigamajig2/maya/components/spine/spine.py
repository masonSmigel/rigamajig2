...
class Spine(base.BaseComponent):
    ...
    def __init__(self, name, input, size=1, rigParent=str(), componentTag=None):
        ...
        self.defineParameter(
            parameter="addSpineMid", value=self.addSpineMid, dataType="bool"
        )
        self.defineParameter(
            parameter="hipsName", value=self.hipsName, dataType="string"
        )
        self.defineParameter(
            parameter="torsoName", value=self.torsoName, dataType="string"
        )
        self.defineParameter(
            parameter="chestName", value=self.chestName, dataType="string"
        )
        self.defineParameter(
            parameter="chestSpaces", value=self.chestSpaces, dataType="dict"
        )
    ...
    def _createBuildGuides(self):
        ...
        self.hipsSwivelGuide = control.createGuide(
            self.name + "_hipSwivel", side=self.side, parent=self.guidesHierarchy
        )
        live.slideBetweenTransforms(
            self.hipsSwivelGuide,
            self.input[1],
            self.input[-2],
            defaultValue=HIPS_PERCENT,
        )

        self.torsoGuide = control.createGuide(
            self.name + "_torso", side=self.side, parent=self.guidesHierarchy
        )

        # setup the slider for the guide
        live.slideBetweenTransforms(
            self.torsoGuide, self.input[1], self.input[-2], defaultValue=TORSO_PERCENT
        )
    ...
    def _rigSetup(self):
        ...
        self.chestTopTrs = hierarchy.create(
            self.chestTop.name, ["{}_trs".format(self.input[0])], above=False
        )[0]
        ...
        # setup the hipSwivel
        self.hipSwingTrs = hierarchy.create(
            self.hipSwing.name, ["{}_trs".format(self.input[0])], above=False
        )[0]
        ...
        cmds.connectAttr(
            "{}.volumeFactor".format(self.chest.name),
        ...
        cmds.connectAttr(
            "{}.tangentVis".format(self.chest.name),
            "{}.v".format(self.chestTangent.orig),
        )
        attr.createAttr(
            self.hipSwing.name,
            "tangentVis",
        ...
        cmds.connectAttr(
            "{}.tangentVis".format(self.hipSwing.name),
            "{}.v".format(self.hipTangent.orig),
        )

        # create the chest piviot offset
        axis = transform.getAimAxis(self.chest.name)
        remap = node.remapValue(
            "{}.{}".format(self.chest.name, "pivotHeight"),
            inMin=0,
            inMax=10,
            outMin=0,
    ...
    def _connect(self):
        ...
        if self.chestSpaces:
            ...
            spaces.addSpace(
                self.chest.spaces, ikspaceValues, self.chestSpaces.keys(), "parent"
            )

        if cmds.objExists(self.rigParent):
            transform.connectOffsetParentMatrix(
                self.rigParent, self.hipSwing.orig, mo=True
            )
            transform.connectOffsetParentMatrix(
                self.rigParent, self.torso.orig, mo=True
            )
            transform.connectOffsetParentMatrix(
                self.rigParent, self.ikspline.getGroup(), mo=True
            )
