...
def create(node, attrHolder=None, parent=None, spaceAttrName='space', defaultName='local'):
    """
    ...
    if cmds.objExists("{}.spaceGroup".format(node)):
        raise RuntimeError("node '{}' already has spaces attatched to it. use addSpace instead".format(node))
    ...
    rig_node.decomposeMatrix("{}.{}".format(node, 'parentMatrix'),
                             outputs=grp, t=True, r=True, s=True,
    ...
    choice = rig_node.choice(selector="{}.{}".format(attrHolder, 'space'),
...
def addSpace(node, targetList, nameList, constraintType='parent'):
    ...
    for target, name in zip(targetList, nameList):
        ...
        existingSpaces = cmds.attributeQuery('space', node=attrHolder, listEnum=True)[0].split(':')
        if name in existingSpaces or target in existingSpaces:
            raise RuntimeError('There is a space with that name in "{}.{}"'.format(attrHolder, 'space'))
        cmds.addAttr('{}.{}'.format(attrHolder, 'space'), e=True, enumName=':'.join(existingSpaces + [name]))

        # create a new space
        newSpace = cmds.createNode('transform', name='{}_{}'.format(node, name), parent=spaceGroup)
